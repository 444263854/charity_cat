<template>
  <article class="detail">
    ...
    <div class="edit_comment">
      ...
      <textarea
        class="edit_con"
        :class="{change_bor_color: isFocus}"
        v-model="comment_edit_text"
        @focus="isFocus = true"
        @blur="isFocus = false"
        placeholder="  说点什么..."
        ref="comment_edit"
    ...
    <div class="comment_box">
      ...
      <div class="load_more">
        <span @click="loadMoreComment" :class="{disable:disable}">加载更多</span>
      </div>
    </div>
  </article>
</template>
    ...
    getArticle() {
      this.axios.get("/article/" + this.$route.params.id).then(res => {
        if (res.data.status == 200) {
          ...
          this.comment_list = this.comment_list.concat(content.comment);
        } else {
    ...
    getComment() {
      this.axios
        ...
        .catch(() => {
          this.disable = false;
          //评论页回退
          this.pages = Math.floor(this.comment_list.length / 10);
        });
    },
    loadMoreComment() {
      if (!this.disable) {
